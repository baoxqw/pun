...
<script>
  import apiAndAtomRelation,{getSelectedListById} from '../../../api/apiInfo/apiAndAtomRelation.js'
  ...
  // crud交由presenter持有
  const defaultCrud = CRUD({ title: '指标授权', url: apiAndAtomRelation.url, sort: 'id,desc', crudMethod: { ...apiAndAtomRelation.crud }})
  const defaultForm = {
    id: null,
    apiId: null,
    bizName: null,
...
/*           atomList: [
    ...
    created() {
     this.form.apiId = this.msg
        getSelectedListById(this.form.apiId).then(data => {
            this.atomList = data;
        })
    },
    mounted() {
    ...
    methods: {
      ...
      [CRUD.HOOK.beforeSubmit]() {
          ...
          console.log("================"+this.form.atomList)
